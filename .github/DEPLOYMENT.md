...
- ?? Образ существует только на production сервере
...
- ?? Требует настройки прав доступа к packages
...
- **Как получить**:
  ```bash
  # На вашем локальном компьютере или на сервере создайте SSH ключ
...
8. ?? Очищает старые образы
9. ?? Проверяет успешность развертывания
